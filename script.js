...
// === Language Management ===
/**
 * Initializes the language based on current selection
 */
...
/**
 * Changes the site language and saves to localStorage
...
/**
 * Updates the UI with translations for the specified language
 * @param {string} lang - Language code
 */
function updateLanguage(lang) {
  // Get translation object for requested language
  const t = translations[lang];
  // If language not found, exit
  if (!t) return;

  // Update all elements with [data-translate] attribute
  document.querySelectorAll('[data-translate]').forEach(el => {
    // Get the dot-notation key (e.g., 'nav.home')
    ...
    if (value) {
      if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') {
        // Handle input placeholders or values
  ...
  // Update elements specifically marked for placeholder translation
  ...
  // Update language selection dropdown UI
  ...
  if (langDropdown) {
    ...
    if (langButton) {
      // Set the button text to current language
      langButton.textContent = langNames[lang] || 'English';
    }
    // Highlight the active language option in the menu
 ...
 * 2. Get your form ID (e.g., https://formspree.io/f/mqkvqoqz)
 * 3. Replace the FORM_ID below with your actual form ID
 */
const FORMSPREE_CONFIG = {
  FORM_ID: 'mqkvqoqz', // Main contact form
  NEWSLETTER_ID: 'mqkvqoqz', // Using same ID or placeholder
  ENDPOINT: 'https://formspree.io/f/'
};

// === Service Details Data for Modals ===
const SERVICE_DETAILS = {
  'webDevelopment': {
    title: 'Web Development',
    content: 'We build high-performance, scalable web applications using React, Node.js, and modern cloud architectures. From e-commerce platforms to custom dashboards, we deliver robust solutions.'
  },
  'aiIntegration': {
    title: 'AI Integration',
    content: 'Leverage the power of Generative AI and Machine Learning. We integrate AI into your workflows, including chatbots, automated data analysis, and predictive models.'
  },
  'appDevelopment': {
    title: 'App Development',
    content: 'Native and cross-platform mobile apps that provide a great user experience on both iOS and Android. We focus on performance, security, and intuitive design.'
  },
  'gameDevelopment': {
    title: 'Game Development',
    content: 'Engaging 2D and 3D gaming experiences powered by Unity and Unreal Engine. We handle everything from concept to deployment on multiple platforms.'
  },
  'cybersecurity': {
    title: 'Cybersecurity',
    content: 'Protect your digital assets with our comprehensive security audits, penetration testing, and secure infrastructure implementation.'
  },
  'cloudMigration': {
    title: 'Cloud Migration',
    content: 'Seamlessly move your infrastructure to AWS, Google Cloud, or Azure. We optimize for cost, performance, and scalability.'
  }
};
...
function initContactForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (FORMSPREE_CONFIG.FORM_ID === 'YOUR_FORM_ID_HERE') {
      showFormMessage(form, 'error', 'Formspree is not configured.');
      return;
    }
    ...
    try {
      const response = await fetch(FORMSPREE_CONFIG.ENDPOINT + FORMSPREE_CONFIG.FORM_ID, {
        method: 'POST',
      ...
      showFormMessage(form, 'error', 'Could not send message. Please try again.');
    } finally {
      if (submitBtn) {
...
function initReviewForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!data.name || !data.rating || !data.message) {
      showFormMessage(form, 'error', 'Please fill in all fields.');
      return;
    }
    ...
    try {
      const response = await fetch(FORMSPREE_CONFIG.ENDPOINT + FORMSPREE_CONFIG.FORM_ID, {
        method: 'POST',
        body: JSON.stringify({ ...data, _subject: 'New Website Review' }),
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json'
        }
      ...
      if (response.ok) {
        showFormMessage(form, 'success', 'Thank you for your review!');
        form.reset();
      } else {
        throw new Error('Failed to submit review');
      }
    } catch (error) {
      showFormMessage(form, 'error', 'Error submitting review. Please try again.');
    } finally {
      if (submitBtn) {
        submitBtn.disabled = false;
        submitBtn.textContent = originalBtnText;
...
function showFormMessage(formElement, type, message) {
  // Remove existing messages for this specific form context
  const existingMessage = formElement.parentNode.querySelector('.form-message');
  if (existingMessage) {
...
function initServiceModals() {
  ...
  if (!modalOverlay || !serviceCards.length) return;

  serviceCards.forEach(card => {
    card.addEventListener('click', () => {
      // Find service key from title text mapping
      const h3 = card.querySelector('h3');
      if (!h3) return;
      const title = h3.textContent.trim();
      const serviceKey = Object.keys(SERVICE_DETAILS).find(key => SERVICE_DETAILS[key].title === title);
...
function initNewsletterForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const response = await fetch(FORMSPREE_CONFIG.ENDPOINT + FORMSPREE_CONFIG.NEWSLETTER_ID, {
        method: 'POST',
        body: JSON.stringify({ email: email, subject: 'New Newsletter Subscription' }),
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json'
        }
...
/*
 * This function handles the visual custom cursor effect.
 * It tracks the mouse movement and updates the position of two elements:
 * 1. .custom-cursor: The small solid dot.
 * 2. .custom-cursor-glow: The larger blurred glow effect.
 */
function initCustomCursor() {
  const cursor = document.querySelector('.custom-cursor');
  const glow = document.querySelector('.custom-cursor-glow');

  // If elements aren't found on the page, exit early
  if (!cursor || !glow) return;

  // Listen for mousemove on the entire document
  document.addEventListener('mousemove', (e) => {
    // Show cursor and glow when mouse moves for the first time
    ...
    // Update positions (centering the elements on the mouse tip)
    cursor.style.transform = `translate(${x - 10}px, ${y - 10}px)`;
    glow.style.transform = `translate(${x - 20}px, ${y - 20}px)`;
  });

  // Scale cursor up when hovering over interactive elements for better UX
  const hoverables = document.querySelectorAll('a, button, .service-card, .faq-item, .glass-card');
  hoverables.forEach(el => {
    el.addEventListener('mouseenter', () => {
      cursor.style.transform += ' scale(2)';
      cursor.style.background = 'var(--accent-purple)'; // Change color to purple on hover
    });
    el.addEventListener('mouseleave', () => {
      // Revert scale and color
...
// === Success Counters Handler ===
/*
 * This function manages the animated numbers that count up from 0 to a target value.
 * It uses the Intersection Observer API to trigger the animation only when the counters
 * become visible on the screen.
 */
function initSuccessCounters() {
  const counters = document.querySelectorAll('.counter-number');
  if (!counters.length) return;

  // Recursive function to increment the counter
  const animateCounter = (counter) => {
    const target = +counter.getAttribute('data-target'); // Final number from data attribute
    const count = +counter.innerText;
    const speed = target / 100; // Adjust increment speed based on the target value
    ...
    if (count < target) {
      ...
      // Ensure it lands exactly on target with a '+' suffix
